...
7. signalScores and signalEvidence MUST STRICTLY ONLY contain keys from this exact list: [${topic?.evaluationSignals?.join(', ') || 'architecture_thinking, implementation_depth, code_quality, tradeoffs, scalability, security'}]. Do NOT invent new signal names. You MUST provide a score for each of these exact exact keys.
...
9. For followUpRecommendations, ensure the questions gradually increase in complexity ONLY IF the user answered well. If the user provided a poor, incorrect, or "I don't know" answer, the follow-ups MUST revert to testing basic fundamentals of the topic.
10. Design follow-ups to take slightly longer to answer than the initial question (e.g., scoping the first follow-up for ~4 minutes, and subsequent ones for ~5 minutes).
