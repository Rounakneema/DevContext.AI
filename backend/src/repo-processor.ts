...
import { ProjectContextMap, DomainInfo } from './types';
import { TokenBudgetManager } from './token-budget-manager';
import { detectDomain } from './domain-utils';
...
export const handler: Handler<ProcessorEvent, ProcessorResponse> = async (event) => {
  ...
  try {
    ...
    // 🕵️ NEW: Hierarchical Domain Detection (Stage 1)
    console.log('🕵️ Detecting primary domain (Stage 1)...');
    const domainInfo = await detectDomain(
      projectContextMap,
      analysisId,
      codeContext
    );
    ...
    if (domainInfo) {
      console.log(`📡 Domain Detected: ${domainInfo.primary_domain} -> ${domainInfo.sub_domain} (Confidence: ${domainInfo.confidence})`);
    } else {
      console.log('⚠️ Domain detection returned null');
    }
